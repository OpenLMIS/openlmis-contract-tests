...
#save logs
mkdir -p build
docker compose -f docker-compose.yml -f ${FILENAME} logs > build/logs
docker compose -f docker-compose.yml -f ${FILENAME} exec nginx cat /etc/nginx/conf.d/default.conf > build/nginx.conf
